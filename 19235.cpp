...
int main() {
    ...
    cout << green + blue;
        ...
        for(int i=4;i<9;i++){
            if(m[row][i+1]){
                m[row][i]=m[row][i-1]=true;
                break;
            }
            if(i==8){
                m[row][9]=m[row][8]=true;
            }
        }
        for(int i=4;i<9;i++){
            if(m[i+1][col]||m[i+1][col+1]){
                m[i][col]=m[i][col+1]=true;
                break;
            }
            if(i==8){
                m[9][col]=m[9][col+1]=true;
            }
        }
    }
    else if (blockCase==3){
        ...
        for(int i=4;i<9;i++){
            cout<<i<<"*";
            printf("row : %d i : %d\n",row,i);
            if(m[row][i+1]||m[row+1][i+1]){
                m[row][i]=m[row+1][i]=true;
                break;
            }
            if(i==8){
                m[row][9]=m[row+1][9]=true;
            }
        }
        for(int i=4;i<9;i++){
            if(m[i+1][col]){
                m[i][col]=m[i-1][col]=true;
                break;
            }
            if(i==8){
                m[9][col]=m[8][col]=true;
            }
        }
    }
}
bool erase(){
    ...
    cout<<"@";
    pivot=-1;
    for(int i=4;i<10;i++){//파란색
        if(m[0][i]&&m[1][i]&&m[2][i]&&m[3][i]){
            m[0][i]=m[1][i]=m[2][i]=m[3][i]=false;
            flag=true;
            pivot=i;
            cnt++;
        }
